...
export class ChartsService {
  ...
  getChartOptions(): any {
    ...
    return {
      ...
      xAxis: {
        type: 'category',
        data: this.getCandleData().categoryData,
        scale: true,
        boundaryGap: false,
  ...
  getCandleData(): any {
    return this.splitData(this.getData());
  }

  calculateMA(dayCount) {
    const result = [];
    for (let i = 0, len = this.getCandleData().values.length; i < len; i++) {
      if (i < dayCount) {
        result.push('-');
        continue;
      }
      ...
      for (let j = 0; j < dayCount; j++) {
        sum += this.getCandleData().values[i - j][1];
      }
